...
import { MessagingUnreadService } from '../../services/messaging-unread.service';
...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  /** Layout plein écran type fil social (route /feed). */
  feedLayout = false;
  username = 'User';
  userAvatar = '';
  ...
  private studentUnreadSub: Subscription | null = null;
  ...
  frontofficeMenuItems: MenuItem[] = [
    ...
    { id: 'forum-general', label: 'Forum général', icon: 'public', routerLink: '/frontoffice/forum/general' },
    { id: 'forum-levels', label: 'Niveaux (A1–C2)', icon: 'school', routerLink: '/frontoffice/forum/levels' },
    { id: 'sec-dm', label: 'Messagerie', icon: 'mail_outline', routerLink: '', kind: 'section' },
    { id: 'messages', label: 'Étudiants & tuteurs', icon: 'chat', routerLink: '/frontoffice/messages' },
    { id: 'sec-inscription', label: 'Inscription', icon: 'how_to_reg', routerLink: '', kind: 'section' },
    { id: 'inscription', label: 'Abonnements', icon: 'loyalty', routerLink: '/frontoffice/inscription' },
    ...
    { id: 'recordings', label: 'Recordings & Highlights', icon: 'video_library', routerLink: '/frontoffice/recordings' },
    ...
    { id: 'evaluations', label: 'Evaluations', icon: 'quiz', routerLink: '/frontoffice/evaluations' },
    { id: 'notebook', label: 'Notebook', icon: 'auto_stories', routerLink: '/frontoffice/notebook' },
  ...
  ngOnInit(): void {
    ...
    this.messagingUnread.startStudentInboxWatch(() => {
      const u = this.authService.getCurrentUser();
      return u?.id != null ? Number(u.id) : 0;
    });
    this.studentUnreadSub = this.messagingUnread.studentUnread$.subscribe(n => {
  ...
  private ensureUnreadPoll(studentId: number): void {
    ...
    this.ngZone.runOutsideAngular(() => {
      this.unreadPollInterval = setInterval(() => {
        this.ngZone.run(() => this.checkUnreadReclamationNotifications(studentId, true));
  ...
  ngOnDestroy(): void {
    ...
    this.studentUnreadSub?.unsubscribe();
    this.studentUnreadSub = null;
