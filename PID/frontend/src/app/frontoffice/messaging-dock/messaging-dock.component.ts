...
export class MessagingDockComponent implements OnInit, OnDestroy {
  ...
  private loadUsers(): void {
    // getAll peut parfois manquer les tuteurs selon le backend; on fusionne avec getByRole('TUTOR').
    forkJoin({
      all: this.usersApi.getAll().pipe(catchError(() => of([] as UserProfile[]))),
      tutors: this.usersApi.getByRole('TUTOR').pipe(catchError(() => of([] as UserProfile[])))
    }).subscribe({
      next: ({ all, tutors }) => {
        const merged = this.mergeUsers(
          Array.isArray(all) ? all : [],
          Array.isArray(tutors) ? tutors : []
        );
        this.users = merged.filter(u => u.id != null && u.id !== 1);
        this.userById = {};
  ...
  private rebuildRoster(): void {
    ...
    // Frontoffice: STUDENT et ADMIN discutent avec les tuteurs.
    // Seul un profil TUTOR voit la liste des étudiants.
    const wantTutors = myRole !== 'TUTOR';
    this.roster = this.users.filter(u => {
      ...
      const onlineA = this.isOnline(a) ? 1 : 0;
      const onlineB = this.isOnline(b) ? 1 : 0;
      if (onlineA !== onlineB) {
        return onlineB - onlineA; // en ligne d'abord
      }
  ...
  rosterStatus(u: UserProfile): string {
    return this.isOnline(u) ? 'En ligne' : 'Hors ligne';
  }
  ...
  private startPolling(): void {
    ...
    this.ngZone.runOutsideAngular(() => {
      this.pollHandle = globalThis.setInterval(() => {
        this.ngZone.run(() => this.loadThread());
      }, 4000);
    });
  }
  ...
  private mergeUsers(primary: UserProfile[], secondary: UserProfile[]): UserProfile[] {
