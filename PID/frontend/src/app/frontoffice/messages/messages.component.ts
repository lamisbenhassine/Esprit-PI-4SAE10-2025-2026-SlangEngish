...
export class MessagesComponent implements OnInit, OnDestroy {
  ...
  private micDeviceId = '';
  private static readonly APPOINTMENT_AGENT_PROMPT = `
You are Riley, an appointment scheduling assistant for Wellness Partners.
Your job is to schedule, confirm, reschedule, or cancel appointments.
Be friendly, clear, and efficient.
Ask one question at a time.
Confirm names, dates, and times explicitly.
For urgent symptoms, triage and suggest emergency care when appropriate.
For new patients, collect full name, date of birth, phone, and explain arrival + documents.
For rescheduling, confirm old slot and new slot.
Close by summarizing appointment details and asking if more help is needed.
`;
  ...
  refreshInbox(): void {
    this.dm.inbox(this.currentUserId).subscribe({
      next: rows => {
        ...
        // Certaines conversations peuvent avoir été créées avec un "kind" incohérent
        // (ex: role check échoue à cause de users-service indisponible).
        // Pour l’affichage, on se base sur le rôle réel de l’autre participant.
        this.inboxTutor = all.filter(r => this.userRole(r.otherUserId) === 'Tutor');
        this.inboxPeer = all.filter(r => this.userRole(r.otherUserId) === 'Student');
        this.tryOpenPendingTarget();
      },
  ...
  openWithTutor(tutor: UserProfile): void {
    ...
    this.dm.open(this.currentUserId, tutor.id, 'WITH_TUTOR').subscribe({
      ...
      error: () => {
        this.loading = false;
        this.snackBar.open(
          'Impossible d’ouvrir la conversation. Vérifiez que gateway (8100), forum (8040) et users sont démarrés.',
          'OK',
          { duration: 7000 }
        );
      }
  ...
  userRole(userId: number): 'Student' | 'Tutor' | 'Team' {
    // Fallback robuste: userById peut être incomplet si l'API users est partielle.
    if (this.tutors.some(t => t.id === userId)) {
      return 'Tutor';
    }
    if (this.inboxTutor.some(r => r.otherUserId === userId)) {
      return 'Tutor';
    }
  ...
  /** Liste rapide pour afficher des tuteurs en ligne même si aucune conversation n'existe encore. */
  onlineTutorsForQuickOpen(): UserProfile[] {
    return this.filteredTutors().filter(t => this.tutorOnline(t));
  }
  ...
  async requestTutorCallFromDiscussion(): Promise<void> {
    ...
    if (this.userRole(this.selected.otherUserId) !== 'Tutor') {
      this.snackBar.open('Appel disponible uniquement avec un tuteur.', 'OK', { duration: 4000 });
      return;
    }
    ...
    try {
      ...
      // Inject runtime behavior to keep the assistant aligned with appointment scheduling flow.
      vapi.send({
        type: 'add-message',
        message: {
          role: 'system',
          content: MessagesComponent.APPOINTMENT_AGENT_PROMPT
        },
      ...
      client.on('call-start', () => {
        ...
        try {
          if (client.getDailyCallObject()) {
            // Keep device selection only; avoid injecting custom audioSource stream
            // (it can break STT after first turn on some browsers).
  ...
  private async ensureMicrophoneAccess(): Promise<void> {
    ...
    const hasLiveTrack = probeStream.getAudioTracks().some(t => t.readyState === 'live');
    if (!hasLiveTrack) {
      probeStream.getTracks().forEach(t => t.stop());
      throw new Error('Aucune piste micro active détectée.');
    }
    const mainTrack = probeStream.getAudioTracks()[0];
    this.micDeviceId = mainTrack?.getSettings?.().deviceId || '';
    // Release probe stream immediately; VAPI opens its own managed stream.
    probeStream.getTracks().forEach(t => t.stop());
  }
