...
const CART_API = `${API_URL}/inscription/cart`;
...
export class CartService {
    ...
    getCartByUserId(userId: number): Observable<Cart> {
        return this.http.get<Cart>(`${CART_API}/${userId}`).pipe(
            tap(cart => this.cartSubject.next(cart))
        );
    }

    addItemToCart(request: AddItemRequest): Observable<Cart> {
        return this.http.post<Cart>(`${CART_API}/add`, request).pipe(
            tap(cart => this.cartSubject.next(cart))
        );
    }
    ...
    addWithSiblingDiscount(userId: number, subscriptionPlanId: number, siblingCount: number): Observable<Cart> {
        return this.http.post<Cart>(`${CART_API}/add-with-sibling-discount`, {
            userId,
            subscriptionPlanId,
            siblingCount
        }).pipe(
    ...
    addWithFamilyRate(userId: number, subscriptionPlanId: number, numberOfPeople: number): Observable<Cart> {
        return this.http.post<Cart>(`${CART_API}/add-with-family-rate`, {
            userId,
            subscriptionPlanId,
            numberOfPeople
        }).pipe(
    ...
    removeItemFromCart(userId: number, itemId: number): Observable<Cart> {
        return this.http.delete<Cart>(`${CART_API}/item/${itemId}/user/${userId}`).pipe(
            tap(cart => this.cartSubject.next(cart))
        );
    }

    removeItemByPlanId(userId: number, subscriptionPlanId: number): Observable<Cart> {
        return this.http.delete<Cart>(`${CART_API}/user/${userId}/plan/${subscriptionPlanId}`).pipe(
            tap(cart => this.cartSubject.next(cart))
        );
    }

    clearCart(userId: number): Observable<void> {
        return this.http.delete<void>(`${CART_API}/${userId}/clear`).pipe(
