...
import esprit.users.dto.SigninFaceRequest;
...
public class UserServiceImpl implements UserService {
    ...
    public User signup(SignupRequest request) {
        ...
        if (request.getFaceDescriptor() != null && !request.getFaceDescriptor().isEmpty()) {
            if (!FaceEmbeddingComparator.isValidDimension(request.getFaceDescriptor())) {
                throw new IllegalArgumentException("Le descripteur facial doit contenir exactement 128 valeurs.");
            }
            try {
                faceJson = objectMapper.writeValueAsString(request.getFaceDescriptor());
            } catch (JsonProcessingException e) {
                throw new IllegalArgumentException("Descripteur facial invalide.");
            }
        }
    ...
    @Override
    public User signinFace(SigninFaceRequest request) {
        User user = userRepository.findByEmail(request.getEmail().trim())
                .orElseThrow(() -> new IllegalArgumentException("Email ou visage non reconnu."));

        if (user.getStatus() == Status.INACTIVE) {
            throw new IllegalStateException("Compte bloqué. Contactez l'administrateur.");
        }

        String stored = user.getFaceDescriptorJson();
        if (stored == null || stored.isBlank()) {
            throw new IllegalArgumentException(
                    "La connexion par visage n'est pas disponible pour ce compte. Utilisez le mot de passe, ou créez un compte avec une photo de visage claire.");
        }

        try {
            List<Double> storedList = objectMapper.readValue(stored, new TypeReference<List<Double>>() {});
            if (!FaceEmbeddingComparator.isValidDimension(storedList)) {
                throw new IllegalStateException("Modèle facial enregistré invalide. Contactez le support.");
            }
            double dist = FaceEmbeddingComparator.euclideanDistance(storedList, request.getFaceDescriptor());
            if (dist > FaceEmbeddingComparator.DEFAULT_MAX_DISTANCE) {
                throw new IllegalArgumentException("Email ou visage non reconnu.");
            }
            return user;
        } catch (JsonProcessingException e) {
            throw new IllegalStateException("Erreur lors de la lecture du modèle facial.", e);
        }
    }

    @Override
    public User signinFaceOnly(SigninFaceOnlyRequest request) {
        List<Double> probe = request.getFaceDescriptor();
        if (!FaceEmbeddingComparator.isValidDimension(probe)) {
            throw new IllegalArgumentException("Le descripteur facial doit contenir 128 valeurs.");
        }

        List<User> candidates = userRepository.findUsersWithFaceDescriptor();
        if (candidates.isEmpty()) {
            throw new IllegalArgumentException("Aucun compte n'a activé la reconnaissance faciale.");
        }

        User bestUser = null;
        Double bestDist = null;
        Double secondBestDist = null;

        for (User u : candidates) {
            if (u.getStatus() == Status.INACTIVE) {
                continue;
            }
            String stored = u.getFaceDescriptorJson();
            if (stored == null || stored.isBlank()) {
                continue;
            }
            try {
                ...
                double d = FaceEmbeddingComparator.euclideanDistance(storedList, probe);
                if (bestDist == null || d < bestDist) {
                    secondBestDist = bestDist;
                    bestDist = d;
                    bestUser = u;
                } else if (secondBestDist == null || d < secondBestDist) {
                    secondBestDist = d;
                }
            } catch (JsonProcessingException ignored) {
                // ignoré : entrée corrompue
            }
        }

        if (bestUser == null) {
            throw new IllegalArgumentException("Aucun visage correspondant n'a été trouvé.");
        }
        if (bestDist > FaceEmbeddingComparator.DEFAULT_MAX_DISTANCE) {
            throw new IllegalArgumentException("Visage non reconnu. Aucun compte ne correspond assez clairement.");
        }
        if (secondBestDist != null
                && secondBestDist <= FaceEmbeddingComparator.DEFAULT_MAX_DISTANCE
                && (secondBestDist - bestDist) < 0.08) {
            throw new IllegalArgumentException(
                    "Plusieurs profils sont trop proches. Utilisez la connexion avec email + visage, ou le mot de passe.");
        }
        return bestUser;
    }
